...
for i=1:size(data,3);             %返回矩阵的波段数   1:�? 2：列
    data(:,:,i) = imfilter(data(:,:,i),fspecial('average',5));%imfilter:多维度滤波器   平均滤波器：大小�?*7
end
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[nr,nc,nb]=size(data);%==================
...
lambda_i  =  [1e-8 2e-8 4e-8 6e-8 8e-8];


for ii_lambda = 1:length(lambda_i)
     
    lambda = lambda_i(ii_lambda);
