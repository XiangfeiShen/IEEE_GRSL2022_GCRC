function class = gcrc(DataTrains, CTrain, DataTests, lambda,gamma,beta)
...
for j = 1: m
   ...
   W = (DDT +  S)\(DataTrain*Y');
    a = 0;
    for i = 1: numClass 
        WW((a+1): (CTrain(i)+a)) = (1./(max(mean(W((a+1): (CTrain(i)+a))),1e-4)));
        a = CTrain(i) + a;
    end
   ...
   %% phase 2
    G = diag(gamma.*WW);
    SS = diag(beta.*norms);
    weights = (DDT +  G + SS)\(DataTrain*Y');
    %weights = (DDT +  G)\(DataTrain*Y');
    %[weights,flag]=pcg(DDT +  G,DataTrain*Y',1e-8,300);
    a = 0;
    for i = 1: numClass 
